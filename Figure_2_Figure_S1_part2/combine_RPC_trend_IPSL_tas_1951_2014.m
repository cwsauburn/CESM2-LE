...
P_corre0 = zeros(500,27);
denomin  = zeros(500,27);
RPC      = zeros(500,27);

filename      = 'Ann_tas_CMIP6_IPSL_RPC_global_continent_1951_2014_trend_1.nc';
...
filename      = 'Ann_tas_CMIP6_IPSL_RPC_global_continent_1951_2014_trend_3.nc';
P_corre0_temp = ncread(filename,'P_corre0');
denomin_temp  = ncread(filename,'denomin');
RPC_temp      = ncread(filename,'RPC');
...
filename      = 'Ann_tas_CMIP6_IPSL_RPC_global_continent_1951_2014_trend_2.nc';
P_corre0_temp = ncread(filename,'P_corre0');
denomin_temp  = ncread(filename,'denomin');
RPC_temp      = ncread(filename,'RPC');

for i = 11:16
    P_corre0(:,i+10) = P_corre0_temp(:,i); 
    denomin(:,i+10)  = denomin_temp(:,i);
    RPC(:,i+10)      = RPC_temp(:,i);   
end  

P_corre0(:,27) = P_corre0_temp(:,17); 
denomin(:,27)  = denomin_temp(:,17);
RPC(:,27)      = RPC_temp(:,17);   

%%

posi = [1:20,21:2:31,32];
...
title('IPSL CM6A LR TAS RPC trend 1951-2014','FontSize',14)
